...
class stance_classifier(nn.Module):
  ...
  def forward(self, x_input_ids, x_seg_ids, x_atten_masks, mask_pos):      
    ...
    for i in range(len(x_input_ids)):
      ...
      predictions += (predicted_mask_token,)
    predictions = torch.stack(predictions).cuda()
    #   a = (torch.dot(predicted_mask_token, self.labels[0]),)
    #   b = (torch.dot(predicted_mask_token, self.labels[1]),)
    #   c = (torch.dot(predicted_mask_token, self.labels[2]),)            
    #   predictions += (torch.stack((a,b,c)).cuda(),)
    ...
    query = self.dropout(predictions)
    linear = self.relu(self.linear(query))
    out = self.out(linear)
