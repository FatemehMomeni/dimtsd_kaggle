...
def convert_data_to_ids(tokenizer, text, target, domain):    
  ...
  for i in range(len(text)):
    prompt = f"The stance of text '{text[i]}' is [MASK] on target '{target[i]}' on domain '{domain[i]}'."
...
def data_helper_bert(x_train_all, x_val_all, x_test_all, model_select):        
  ...
  x_train, y_train, x_train_tar, x_train_domain = x_train_all[0], x_train_all[1], x_train_all[2], x_train_all[3]
  x_val, y_val, x_val_tar, x_val_domain = x_val_all[0], x_val_all[1], x_val_all[2], x_val_all[3]
  x_test, y_test, x_test_tar, x_test_domain = x_test_all[0], x_test_all[1], x_test_all[2], x_test_all[3]
  ...
  train_input_ids, train_seg_ids, train_atten_masks, train_mask_pos = convert_data_to_ids(tokenizer, x_train, x_train_tar, x_train_domain)
